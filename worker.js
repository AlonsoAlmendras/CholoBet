...
const DEFAULT_ADMIN_PASS = 'cholobet2026'; // Override with ADMIN_PASS in production.
...
function adminPassFor(env){ return env.ADMIN_PASS || DEFAULT_ADMIN_PASS; }
...
async function handleResults(env, request){
  ...
  if(adminPass !== adminPassFor(env)) return json({ok: false, error: 'Clave de admin incorrecta'}, 403);
  const fx = await getFixtures(env);
  const f = fx.fechas.find(x => x.id === fechaId);
  if(!f) return json({ok: false, error: 'Fecha no existe'});
